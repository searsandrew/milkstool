...
class RefreshCustomerBalance implements ShouldBeUnique, ShouldQueue
{
    use Queueable;
    ...
    public function handle(SyncCustomerBalance $sync): void
    {
    ...
    public function failed(?Throwable $exception): void
    {
        Company::query()->where('netsuite_id', $this->customerId)
            ->where(fn (Builder $query) => $query->whereNull('balance_synced_at')->orWhere('balance_synced_at', '<=', $this->requestedAt))
            ->update([
                'balance_sync_error' => 'Background refresh failed. Inspect failed queue jobs before retrying.',
                'balance_next_sync_at' => now()->addHours(6),
            ]);
    }
}
