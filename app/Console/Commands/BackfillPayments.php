...
{
    protected $signature = 'milkstool:backfill-payments {--limit=3 : Maximum customers to import and reconcile} {--dry-run : List the next customers without importing}';
    ...
    public function handle(): int
    {
        ...
        $lock = Cache::lock('netsuite-payment-backfill', 86400);

        if (! $lock->get()) {
            $this->error('A payment backfill is already running.');

            return self::FAILURE;
        }

        try {
            $companies = Company::query()->where('is_active', true)->whereNull('payments_backfilled_at')
            ...
            foreach ($companies as $company) {
                ...
                if ($this->call('milkstool:sync-payments', ['customer' => $company->netsuite_id]) !== self::SUCCESS) {
