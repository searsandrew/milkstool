...
{
    public function __construct(private CreditMemoSource $source) {}
    ...
    public function compare(Company $company): array
    {
        ...
        $currencies = $sourceCreditMemos->keys()->merge($sourceLines->keys())
            ->merge($localCreditMemos->keys())->merge($localLines->keys())->unique()->sort();
        $results = [];

        foreach ($currencies as $currency) {
            $sourceValues = array_merge($sourceCreditMemos->get($currency, []), $sourceLines->get($currency, []));
            $localValues = array_merge((array) $localCreditMemos->get($currency), (array) $localLines->get($currency));

            foreach (['paid_count', 'unpaid_count', 'foreign_amount_paid', 'foreign_amount_unpaid', 'credit_memo_count', 'line_count', 'quantity_count', 'amount_count', 'total', 'foreign_total', 'quantity', 'amount', 'detail_amount'] as $metric) {
