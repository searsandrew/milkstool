...
class BackfillCreditMemos extends Command
{
    protected $signature = 'milkstool:backfill-credit-memos {--limit=3 : Maximum customers to import and reconcile} {--dry-run : List the next customers without importing}';
    ...
    public function handle(): int
    {
        ...
        $lock = Cache::lock('netsuite-credit-memo-backfill', 86400);

        if (! $lock->get()) {
            $this->error('A credit memo backfill is already running.');

            return self::FAILURE;
        }

        try {
            $companies = Company::query()->where('is_active', true)->whereNull('credit_memos_backfilled_at')
            ...
            foreach ($companies as $company) {
                ...
                if ($this->call('milkstool:sync-credit-memos', ['customer' => $company->netsuite_id]) !== self::SUCCESS) {
